...
########################  PSO  ##################################
#轉成存文字
mycorpus=Corpus(DirSource("C:/data/fb_pdata"),list(language=NA))
...
fno=dbGetQuery(con, "select message from firstdata WHERE target=0 AND affect=0 limit 0,100")
fp=dbGetQuery(con, "select message from firstdata WHERE target=1 AND affect=0 limit 0,100")
first_data=rbind(fn,fno,fp)
fnn=dbGetQuery(con, "select id from firstdata WHERE target=-1 AND affect=0")
fnon=dbGetQuery(con, "select id from firstdata WHERE target=0 AND affect=0 limit 0,100")
fpn=dbGetQuery(con, "select id from firstdata WHERE target=1 AND affect=0 limit 0,100")
first_data_name=rbind(fnn,fnon,fpn)

fnnt=dbGetQuery(con, "select target from firstdata WHERE target=-1 AND affect=0")
fnot=dbGetQuery(con, "select  target from firstdata WHERE target=0 AND affect=0 limit 0,100")
fpnt=dbGetQuery(con, "select target from firstdata WHERE target=1 AND affect=0 limit 0,100")
type=rbind(fnnt,fnot,fpnt)
'
