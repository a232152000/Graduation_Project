...
##抓資料 first留言 + 回櫃的留言
fp=dbGetQuery(con, "select message from firstdata WHERE target=1  AND affect=0")
fpn=dbGetQuery(con, "select id from firstdata WHERE target=1  AND affect=0")
fpnt=dbGetQuery(con, "select target from firstdata WHERE target=1  AND affect=0")
pp=dbGetQuery(con, "select PostMessage from test WHERE POST_FeedBack=1 AND number<=1000 AND affect=0")
ppn=dbGetQuery(con, "select PostId from test WHERE POST_FeedBack=1 AND number<=1000 AND affect=0")
ppnt=dbGetQuery(con, "select POST_FeedBack from test WHERE POST_FeedBack=1 AND number<=1000 AND affect=0")
...
##抓資料 first留言 + 回櫃的留言
fn=dbGetQuery(con, "select message from firstdata WHERE target=-1 AND affect=0")
fnn=dbGetQuery(con, "select id from firstdata WHERE target=-1  AND affect=0")
fnnt=dbGetQuery(con, "select target from firstdata WHERE target=-1  AND affect=0")
pn=dbGetQuery(con, "select PostMessage from test WHERE POST_FeedBack=-1 AND number<=1000 AND affect=0")
pnn=dbGetQuery(con, "select PostId from test WHERE POST_FeedBack=-1 AND number<=1000 AND affect=0")
pnnt=dbGetQuery(con, "select POST_FeedBack from test WHERE POST_FeedBack=-1 AND number<=1000 AND affect=0")
...
rf=apply(rfmatrix, 1, function(x) {
  if(is.na(x[2])) {
   c(term=unname(x[1]), RF=log((0/as.numeric(x[3]))+1))
    ...
    c(term=unname(x[1]),  RF=log((as.numeric(x[2])/1)+1))
  }
  else{
...
nnt=dbGetQuery(con, "select POST_FeedBack from test WHERE number<=1000 AND affect=0")
...
if(is.na(x[data_row])){
x[data_row]=log(2)
#這邊乘的rf 會從0轉為1 有錯8/16 as.numeric(rf3[nrow(rf3)-5,])
...
a=cbind(tfrfidfx21,tfrfidfx31,tfrfidfx51)
write.table(tfrfidfsvm1, file = "C:/Users/user/Desktop/104/R學長姐/暑假/程式碼/第一次訓練10次/2.csv", sep = ",", col.names = NA)
write.table(a, file = "C:/Users/user/Desktop/104/R學長姐/暑假/程式碼/第一次訓練10次/2_1.csv", sep = ",", col.names = NA)
